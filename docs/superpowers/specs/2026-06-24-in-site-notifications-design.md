...
- **方案 A：自建 SMTP（nodemailer + QQ/163 企业邮）**——免费但日 50-200 上限、容易进垃圾箱、验证码丢失 = 用户流失
- **方案 B：阿里云 DirectMail**——到达率高但需 ICP 备案域名（7-15 天）
- **方案 C：抽象 Mailer 接口 + 多驱动**——架构灵活但要等备案
- ❌ **结论**：MVP 阶段 3 个方案都不合适，**先不做邮件**
...
- **邮件 fallback**：抽象 Mailer 接口，对重要 category 同时发邮件
