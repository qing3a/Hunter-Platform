...
四个相对独立的协议稳定化 — **breaking change**，Agent 接入前必读：

- **R1.C2** — 长 session + 多角色认证
  - `POST /v1/auth/login` 拿 `sess_*`（168h 滑动 TTL）；`POST /v1/auth/refresh` 续期；`POST /v1/auth/logout` 撤销（idempotent）
  - 新用户注册即获三角色（`candidate` / `hr` / `pm`），`X-Active-Role: pm|hr|candidate` header 切换活跃角色
  - 旧 `hp_live_*` API key 仍可用（向后兼容），但新 agent 优先用 session
  - **BREAKING**: `user_type` 枚举重命名 `headhunter → hr`、`employer → pm`（生产 SQLite rebuild，见 `OPERATIONS.md` §3.3）
  - 详见 §1.1 session token + §1.2 Active Role 约束

- **R1.C3** — 入站 webhook 去重
  - 新端点 `POST /v1/webhooks/qing3`，HMAC 验证后 `INSERT OR IGNORE` 进 `webhook_inbox_deliveries` 表
  - 重放窗口 ±300s = ±5min；body `UNIQUE(endpoint, body_hash)` 去重（同 body 重投返回同一 `delivery_id`，`deduped: true`）
  - 详见 §2.1 第 6 行 + §17 Collab Mode

- **R1.C4** — Capability aliases（为 ow-recruit 桥接而设计）
  - `Capability.aliases?: readonly string[]` 字段；外部客户端可以用自己的命名
  - 服务端查表：`GET /v1/capabilities/by-alias/:name` → `{ canonical, method, path }`（公开端点，已实装）
  ...
  - 详见 §2.1.0.1 别名表 + §17 Collab Mode

- **T10** — `roleGate()` 中间件闭包 RBAC
  - 挂在 `/v1/pm`、`/v1/employer`、`/v1/employer-panel`（pm-gated），`/v1/headhunter`、`/v1/headhunter-workspace`（hr-gated），`/v1/candidate`（candidate-gated）
  - 同一 user 用不同 `X-Active-Role` token 调不同 prefix 的路由会被 403 — 这是预期行为，不是 bug
  - 详见 §1.2 Active Role 约束
