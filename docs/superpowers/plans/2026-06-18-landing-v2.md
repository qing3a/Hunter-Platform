...
**Goal:** Upgrade `GET /` with 3 social proof sections + full visual refresh (animations, brand colors, hover effects).

**Architecture:** Modify existing landing template + router. Add inline JS for countup animations. No new dependencies, no DB schema changes.

**Tech Stack:** TypeScript, Express, node:sqlite, vitest, inline vanilla JS.
...
| File | Action |
|------|--------|
| `src/main/routes/landing.ts` | Modify — add 3 new data queries |
| `src/main/modules/view/templates/landing.ts` | Modify — new sections + CSS + inline JS |
| `tests/integration/landing.test.ts` | Modify — add 6 new tests |

---

## Existing Code Reference (READ FIRST)

1. `src/main/modules/view/templates/landing.ts` — current v1 template
2. `src/main/routes/landing.ts` — current v1 router
3. Run `node --import tsx -e "import {openDb} from './src/main/db/connection'; const db = openDb('tmp/hunter.db'); console.log('placements:', db.prepare('PRAGMA table_info(placements)').all());"` to verify columns

---

## Task 1: Add 3 data queries to landing router

**Files:**
- Modify: `src/main/routes/landing.ts`
- Modify: `src/main/modules/view/templates/landing.ts` (interfaces)

- [ ] **Step 1: Verify schema**

```bash
cd D:\dev-hunter-platform
node --import tsx -e "import {openDb} from './src/main/db/connection'; const db = openDb('tmp/hunter.db'); console.log('placements:', db.prepare('PRAGMA table_info(placements)').all());"
```

Note actual column names. Adjust queries accordingly.

- [ ] **Step 2: Add new interfaces to template**

Open `src/main/modules/view/templates/landing.ts`. Add new interfaces near existing ones:
...
Update `LandingData` interface — add fields:
...
- [ ] **Step 3: Add queries + helpers to router**

Open `src/main/routes/landing.ts`. In `gatherLandingData`, add BEFORE the `return`:
...
```typescript
  ...
  // Latest 5 placements (joined with job + headhunter)
...
Add helper functions OUTSIDE the closure:
```typescript
...
Add import at top of landing.ts router:
```typescript
import type { HeadhunterRanking, PlacementItem, ... } from '../modules/view/templates/landing.js';
```
...
## Task 2: Rewrite template with new sections + visual refresh

**Files:**
- Modify: `src/main/modules/view/templates/landing.ts`

- [ ] **Step 1: Replace renderLanding + add helpers**

Replace the entire `renderLanding` function with:
...
## Task 3: Add 6 new tests

**Files:**
- Modify: `tests/integration/landing.test.ts`
...
```typescript
  describe('Hero Stats section', () => {
    it('shows today\'s unlocks count', async () => {
      ...
      expect(res.text).toMatch(/今日解锁[\s\S]{0,200}data-target="[1-9]/);
    });
  });
...
cd D:\dev-hunter-platform && pnpm api:dev > tmp/landing-v2-smoke.log 2>&1 &
sleep 5

curl -sS -w "\n[%{http_code}] content-type=%{content_type} bytes=%{size_download}\n" "http://localhost:3000/" 2>&1 | tail -3
curl -sS "http://localhost:3000/" 2>&1 | grep -oE "(Hero Stats|Top 3 Headhunters|Latest 5 Placements|For Employers|For Headhunters|For Candidates|🔓|🥇|🎯|--brand-primary)" | sort -u
```

Expected: HTTP 200, all sections + brand color present.
...
cd D:\dev-hunter-platform
git add -A
git commit -m "feat(landing): v2 — social proof + visual refresh

3 new sections:
- Hero Stats: 4 live counters (今日解锁/placements/candidates/uptime) with CountUp animation
- Top 3 Headhunters: leaderboard preview with medals
- Latest 5 Placements: recent successful unlocks (anonymized company)

Visual refresh:
- Brand palette (teal primary + warm amber accent)
...
All CSS-driven; ~40 lines inline vanilla JS for countUp + smooth scroll.
No new dependencies. No PII exposed."
...
- Spec coverage: T1 (queries) + T2 (template) + T3 (tests) + T4 (smoke)
- No placeholders. T1 Step 1 says "verify schema" (action item, not placeholder).
- Type consistency: `HeadhunterRanking` + `PlacementItem` defined in T1 Step 2, used in T1 Step 3 + T2.

## Execution Handoff

Plan saved to `docs/superpowers/plans/2026-06-18-landing-v2.md`. Two options:

1. Subagent-Driven (recommended)
2. Inline Execution

Estimated: 1 commit, ~500 lines, ~45 min.