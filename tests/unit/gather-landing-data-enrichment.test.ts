...
describe('gatherLandingData - industryNav (SQL A)', () => {
  ...
  it('aggregates open jobs by industry, sorted DESC, limited to 20', () => {
    db.exec(`
      ...
      VALUES
        ('u_e1', 'pm', 'E1', 'e@e.com', 'active', 50, 'h1', 'p1', datetime('now'), datetime('now'), datetime('now')),
        ('u_h1', 'hr', 'H1', 'h@h.com', 'active', 50, 'h2', 'p2', datetime('now'), datetime('now'), datetime('now'));
  ...
  it('excludes jobs with NULL industry', () => {
    db.exec(`
      INSERT INTO users (id, user_type, name, contact, status, reputation, api_key_hash, api_key_prefix, quota_reset_at, created_at, updated_at)
      VALUES ('u_e1', 'pm', 'E1', 'e@e.com', 'active', 50, 'h1', 'p1', datetime('now'), datetime('now'), datetime('now'));
      INSERT INTO jobs (id, employer_id, title, status, industry, created_at, updated_at)
...
describe('gatherLandingData - featuredJobs (SQL B)', () => {
  ...
  it('returns 10 open jobs sorted by priority ASC then created_at DESC', () => {
    db.exec(`
      INSERT INTO users (id, user_type, name, contact, status, reputation, api_key_hash, api_key_prefix, quota_reset_at, created_at, updated_at)
      VALUES ('u_e1', 'pm', 'Boss Inc', 'e@e.com', 'active', 50, 'h1', 'p1', datetime('now'), datetime('now'), datetime('now'));
  ...
  it('LEFT JOIN handles orphan job (employer_id NULL) — excluded', () => {
    db.exec(`
      INSERT INTO users (id, user_type, name, contact, status, reputation, api_key_hash, api_key_prefix, quota_reset_at, created_at, updated_at)
      VALUES ('u_h1', 'hr', 'H1', 'h@h.com', 'active', 50, 'h2', 'p2', datetime('now'), datetime('now'), datetime('now'));
...
describe('gatherLandingData - hotCompanies (SQL C)', () => {
  ...
  it('ranks employers by open job count DESC, limits to 4', () => {
    db.exec(`
      INSERT INTO users (id, user_type, name, contact, status, reputation, api_key_hash, api_key_prefix, quota_reset_at, created_at, updated_at)
      VALUES
        ('u_e1', 'pm', 'Boss Inc', 'e1@e.com', 'active', 50, 'h1', 'p1', datetime('now'), datetime('now'), datetime('now')),
        ('u_e2', 'pm', 'Acme',     'e2@e.com', 'active', 50, 'h2', 'p2', datetime('now'), datetime('now'), datetime('now')),
        ('u_e3', 'pm', 'OldCo',    'e3@e.com', 'suspended', 50, 'h3', 'p3', datetime('now'), datetime('now'), datetime('now'));
...
describe('gatherLandingData - demo data isolation (prod mode)', () => {
  ...
  function seedMixedData() {
    db.exec(`
      INSERT INTO users (id, user_type, name, contact, status, reputation, api_key_hash, api_key_prefix, quota_reset_at, created_at, updated_at)
      VALUES
        ('real_emp',   'pm',   'Real Co',  'r@r.com', 'active', 50, 'h_r', 'p_r', datetime('now'), datetime('now'), datetime('now')),
        ('demo_emp',   'pm',   'Demo Co',  'd@d.com', 'active', 50, 'h_d', 'p_d', datetime('now'), datetime('now'), datetime('now'));
...
describe('gatherLandingData - demo data isolation (dev mode)', () => {
  ...
  it('shows demo data in dev mode (no filter)', () => {
    db.exec(`
      INSERT INTO users (id, user_type, name, contact, status, reputation, api_key_hash, api_key_prefix, quota_reset_at, created_at, updated_at)
      VALUES ('demo_emp', 'pm', 'Demo Co', 'd@d.com', 'active', 50, 'h_d', 'p_d', datetime('now'), datetime('now'), datetime('now'));
      INSERT INTO jobs (id, employer_id, title, status, industry, created_at, updated_at)
