...
### Conformance tooling — stub filtering + smart gen + 5 deep PM scenarios (PR #6)

Closes the v1.10 "long-term bloat" follow-up outlined in PR #5:

**1. `conformance:check` is now honest about stubs.** Added a `stripStubs()` pass in `scripts/check-conformance-coverage.ts` that filters out `it.todo(...)` / `it.skip(...)` lines before scanning for capability mentions. **Before** this change, the generated `_generated.test.ts` placeholders were falsely counted as scenarios. **After**, only REAL test references count toward coverage.

**2. `conformance:gen` is now smart.** Reuses the same strip-stubs scanner as the check, but in reverse: it scans every other `*.test.ts` in `skill-md-conformance/` and skips emitting a stub for any capability that is already covered. Re-running the generator on a codebase where all 132 caps have real tests now produces a file with **0 stubs** (just the header comment block). Output is still deterministic and idempotent.

**3. `_generated.test.ts` deleted.** With smart gen + honest check, the file no longer serves a purpose. Removed from git; the conformance gate (`pnpm conformance:check`) is unaffected and still passes 132/132.

**4. 5 deep behavioral scenarios for PM router caps** in a new `tests/integration/skill-md-conformance/pm-deep.test.ts`:
- `pm.list_matches` end-to-end: create project + position → register candidate → recompute → GET matches → returns the candidate row
- `pm.list_matches` with `?min_score=80` filter
- `pm.list_matches` with `?limit=1` pagination
- `pm.snapshot` reflects both seeded projects + position counts
- `pm.position_sandbox` returns 6-stage funnel for a real position

**Verification**:
- `pnpm typecheck` — 0 errors
- `pnpm capabilities:check` — 0 issues
- `pnpm conformance:check` — 132/132 caps covered (with stub-stripping)
- `pnpm openapi:check` — 0 forward gaps
- `pnpm vitest run tests/integration/skill-md-conformance/pm-deep.test.ts` — 5/5 PASS
- `pnpm vitest run tests/integration/skill-md-conformance/v1.10-conformance-smoke.test.ts` — 42/42 still PASS

**Risks acknowledged**:
- Smart gen output is still regenerated by `pnpm conformance:gen` if invoked. The auto-emit is purely additive — running it on a partial codebase emits stubs for the missing caps only. No accidental over-write of real tests.
