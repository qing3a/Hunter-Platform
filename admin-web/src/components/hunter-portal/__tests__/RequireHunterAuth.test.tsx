...
import { RequireHunterAuth } from '../RequireHunterAuth';
...
describe('RequireHunterAuth', () => {
  ...
  it('redirects to /hunter/login when there is no session', () => {
    render(
      <MemoryRouter initialEntries={['/hunter/workspace']}>
        <RequireHunterAuth>
          <div data-testid="content">hunter content</div>
        </RequireHunterAuth>
      </MemoryRouter>,
    );

    expect(lastNavigateTo).toBe('/hunter/login');
    expect(screen.queryByTestId('content')).toBeNull();
  });

  it('renders children when session.role is "headhunter"', () => {
    setSession({
      api_key: 'hp_live_abc',
      user_id: 'u-hunter',
    ...
    expect(screen.getByTestId('content')).toHaveTextContent('hunter content');
  });
  ...
  it('redirects to /candidate/login for non-headhunter roles (pm, employer)', () => {
    for (const role of ['pm', 'employer'] as const) {
      cleanup();
      lastNavigateTo = undefined;
      lastNavigateState = undefined;
      clearSession();
